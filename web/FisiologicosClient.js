...
// Model for Fisiologicos entity
    models.Fisiologicos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/fisiologicos/",
        idAttribute: 'id',
        defaults: {
            medicion: "",
            cedula: ""
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/fisiologicos/';
            }
    ...
    // Collection class for Fisiologicos entities
    models.FisiologicosCollection = Backbone.Collection.extend({
        model: models.Fisiologicos,
        url: "http://localhost:8080/RecuperAppServer/WebServices/fisiologicos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FisiologicosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-fisiologicos-list-itemis template identifier for item
                        templateName: '#tpl-fisiologicos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.fisiologicos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.fisiologicos,
                    // tpl-fisiologicos-details is template identifier for chosen model element
                    templateName: '#tpl-fisiologicos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Fisiologicos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-fisiologicos-details is a template identifier for chosen model element
                templateName: '#tpl-fisiologicos-details',
