...
// Model for ListaSintomas entity
    models.ListaSintomas = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/listasintomas/",
        idAttribute: 'idSintoma',
        defaults: {
            sintoma: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idSintoma');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/listasintomas/';
            }
    ...
    // Collection class for ListaSintomas entities
    models.ListaSintomasCollection = Backbone.Collection.extend({
        model: models.ListaSintomas,
        url: "http://localhost:8080/RecuperAppServer/WebServices/listasintomas/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ListaSintomasCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-listasintomas-list-itemis template identifier for item
                        templateName: '#tpl-listasintomas-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.listasintomas = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.listasintomas,
                    // tpl-listasintomas-details is template identifier for chosen model element
                    templateName: '#tpl-listasintomas-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ListaSintomas();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-listasintomas-details is a template identifier for chosen model element
                templateName: '#tpl-listasintomas-details',
