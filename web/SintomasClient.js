...
// Model for Sintomas entity
    models.Sintomas = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/sintomas/",
        idAttribute: 'id',
        defaults: {
            idSintoma: "",
            cedula: ""
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/sintomas/';
            }
    ...
    // Collection class for Sintomas entities
    models.SintomasCollection = Backbone.Collection.extend({
        model: models.Sintomas,
        url: "http://localhost:8080/RecuperAppServer/WebServices/sintomas/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.SintomasCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-sintomas-list-itemis template identifier for item
                        templateName: '#tpl-sintomas-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.sintomas = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.sintomas,
                    // tpl-sintomas-details is template identifier for chosen model element
                    templateName: '#tpl-sintomas-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Sintomas();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-sintomas-details is a template identifier for chosen model element
                templateName: '#tpl-sintomas-details',
