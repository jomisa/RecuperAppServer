...
// Model for ListaMedicamentos entity
    models.ListaMedicamentos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/listamedicamentos/",
        idAttribute: 'idMedicamento',
        defaults: {
            idSintoma: "",
            dosis: "",
            nombre: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idMedicamento');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/listamedicamentos/';
            }
    ...
    // Collection class for ListaMedicamentos entities
    models.ListaMedicamentosCollection = Backbone.Collection.extend({
        model: models.ListaMedicamentos,
        url: "http://localhost:8080/RecuperAppServer/WebServices/listamedicamentos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ListaMedicamentosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-listamedicamentos-list-itemis template identifier for item
                        templateName: '#tpl-listamedicamentos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.listamedicamentos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.listamedicamentos,
                    // tpl-listamedicamentos-details is template identifier for chosen model element
                    templateName: '#tpl-listamedicamentos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ListaMedicamentos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-listamedicamentos-details is a template identifier for chosen model element
                templateName: '#tpl-listamedicamentos-details',
