...
// Model for Caminatas entity
    models.Caminatas = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/caminatas/",
        idAttribute: 'id',
        defaults: {
            idSintoma: "",
            tiempo: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/caminatas/';
            }
    ...
    // Collection class for Caminatas entities
    models.CaminatasCollection = Backbone.Collection.extend({
        model: models.Caminatas,
        url: "http://localhost:8080/RecuperAppServer/WebServices/caminatas/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CaminatasCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-caminatas-list-itemis template identifier for item
                        templateName: '#tpl-caminatas-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.caminatas = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.caminatas,
                    // tpl-caminatas-details is template identifier for chosen model element
                    templateName: '#tpl-caminatas-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Caminatas();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-caminatas-details is a template identifier for chosen model element
                templateName: '#tpl-caminatas-details',
