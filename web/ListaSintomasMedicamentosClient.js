...
// Model for ListaSintomasMedicamentos entity
    models.ListaSintomasMedicamentos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/listasintomasmedicamentos/",
        idAttribute: 'idSintomaMedicamento',
        defaults: {
            sintoma: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idSintomaMedicamento');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/listasintomasmedicamentos/';
            }
    ...
    // Collection class for ListaSintomasMedicamentos entities
    models.ListaSintomasMedicamentosCollection = Backbone.Collection.extend({
        model: models.ListaSintomasMedicamentos,
        url: "http://localhost:8080/RecuperAppServer/WebServices/listasintomasmedicamentos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ListaSintomasMedicamentosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-listasintomasmedicamentos-list-itemis template identifier for item
                        templateName: '#tpl-listasintomasmedicamentos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.listasintomasmedicamentos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.listasintomasmedicamentos,
                    // tpl-listasintomasmedicamentos-details is template identifier for chosen model element
                    templateName: '#tpl-listasintomasmedicamentos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ListaSintomasMedicamentos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-listasintomasmedicamentos-details is a template identifier for chosen model element
                templateName: '#tpl-listasintomasmedicamentos-details',
        ...
        getData: function () {
            return {
                idSintomaMedicamento: $('#idSintomaMedicamento').val(),
                sintoma: $('#sintoma').val()
            };
        }
    });
