...
// Model for Medicamentos entity
    models.Medicamentos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/medicamentos/",
        idAttribute: 'id',
        defaults: {
            idMedicamento: "",
            idSintoma: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/medicamentos/';
            }
    ...
    // Collection class for Medicamentos entities
    models.MedicamentosCollection = Backbone.Collection.extend({
        model: models.Medicamentos,
        url: "http://localhost:8080/RecuperAppServer/WebServices/medicamentos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MedicamentosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-medicamentos-list-itemis template identifier for item
                        templateName: '#tpl-medicamentos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.medicamentos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.medicamentos,
                    // tpl-medicamentos-details is template identifier for chosen model element
                    templateName: '#tpl-medicamentos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Medicamentos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-medicamentos-details is a template identifier for chosen model element
                templateName: '#tpl-medicamentos-details',
