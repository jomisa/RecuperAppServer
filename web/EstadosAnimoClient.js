...
// Model for EstadosAnimo entity
    models.EstadosAnimo = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/estadosAnimo/",
        idAttribute: 'id',
        defaults: {
            cedula: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/estadosAnimo/';
            }
    ...
    // Collection class for EstadosAnimo entities
    models.EstadosAnimoCollection = Backbone.Collection.extend({
        model: models.EstadosAnimo,
        url: "http://localhost:8080/RecuperAppServer/WebServices/estadosAnimo/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.EstadosAnimoCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-estadosanimo-list-itemis template identifier for item
                        templateName: '#tpl-estadosanimo-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.estadosanimo = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.estadosanimo,
                    // tpl-estadosanimo-details is template identifier for chosen model element
                    templateName: '#tpl-estadosanimo-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.EstadosAnimo();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-estadosanimo-details is a template identifier for chosen model element
                templateName: '#tpl-estadosanimo-details',
