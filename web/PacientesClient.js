...
// Model for Pacientes entity
    models.Pacientes = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RecuperAppServer/WebServices/pacientes/",
        idAttribute: 'id',
        defaults: {
            apellidos: "",
            cedula: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RecuperAppServer/WebServices/pacientes/';
            }
    ...
    // Collection class for Pacientes entities
    models.PacientesCollection = Backbone.Collection.extend({
        model: models.Pacientes,
        url: "http://localhost:8080/RecuperAppServer/WebServices/pacientes/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PacientesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-pacientes-list-itemis template identifier for item
                        templateName: '#tpl-pacientes-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.pacientes = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.pacientes,
                    // tpl-pacientes-details is template identifier for chosen model element
                    templateName: '#tpl-pacientes-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Pacientes();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-pacientes-details is a template identifier for chosen model element
                templateName: '#tpl-pacientes-details',
